...
class CursorConfigCommand extends Command
{
    ...
    protected $description = 'Create or update .cursor/mcp.json with the laravel-boost MCP entry (merges with existing servers)';

    public function handle(): int
    {
        if (CursorMcpConfig::writeOrMerge(base_path())) {
            $this->info('Created/updated '.CursorMcpConfig::getPath(base_path()));
            $this->line('Open this Laravel app folder in Cursor and enable the laravel-boost MCP server.');
