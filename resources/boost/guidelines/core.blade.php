...
**Dependency model:** `Instance-[:DEPENDS_ON {file,line,via,type,method,parameter,helper}]->Dependency-[:RESOLVES_TO {lifetime}]->Identifier`. `DEPENDS_ON.type`: `constructor_injection`, `method_injection` (controller actions, `handle()` on jobs/commands/listeners/middleware), or `global_helper`. `Dependency.access`: `di`, `facade`, `global_helper`, `service_location`. `RESOLVES_TO.lifetime`: `singleton` or `bind`. MCP dependency entries expose `access`, `lifetime`, method-injection metadata when present, and `helper` for global helpers. Bindings: `BINDS_TO` with `normal` or `singleton` on `:Abstract` nodes. Contextual `when/needs/give` overrides: `Instance-[:CONTEXTUAL_BINDS {needs,needs_kind,reason}]->Identifier`. Re-run `container:graph` after upgrades.
