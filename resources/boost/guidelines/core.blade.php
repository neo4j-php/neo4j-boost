...
**Dependency model (SOFT-58):** `Instance-[:DEPENDS_ON {file,line,via}]->Dependency-[:RESOLVES_TO {lifetime}]->Identifier`. `Dependency.access`: `di`, `facade`, `global_helper`, `service_location`. `RESOLVES_TO.lifetime`: `singleton` or `bind`. MCP dependency entries expose `access` and `lifetime`. Bindings: `BINDS_TO` with `normal` or `singleton` on `:Abstract` nodes. Contextual `when/needs/give` overrides: `Instance-[:CONTEXTUAL_BINDS {needs,needs_kind,reason}]->Identifier` (SOFT-49). Re-run `container:graph` after upgrades.
