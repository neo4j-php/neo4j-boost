...
**Dependency model (SOFT-58):** `Instance-[:DEPENDS_ON {file,line,via,type,method,parameter,helper}]->Dependency-[:RESOLVES_TO {lifetime}]->Identifier`. `DEPENDS_ON.type`: `constructor_injection`, `method_injection` (controller actions, `handle()` on jobs/commands/listeners/middleware), or `global_helper`. `Dependency.access`: `di`, `facade`, `global_helper`, `service_location`. `RESOLVES_TO.lifetime`: `singleton` or `bind`. MCP dependency entries expose `access`, `lifetime`, method-injection metadata when present, and `helper` for global helpers. Bindings: `BINDS_TO` with `normal` or `singleton` on `:Abstract` nodes — re-run `container:graph` after upgrades.
...
**Static scan (hidden dependencies):** set `NEO4J_CONTAINER_GRAPH_STATIC_SCAN_PATHS` to comma-separated absolute paths (e.g. `base_path('app/Services')`). The scan adds `DEPENDS_ON` edges with `source: static` for **service_location** (`app()`, `resolve()`, `App::make()` with literal class args), **facade** (`Cache::put`, custom `Facade::method`, resolved via the resolution catalog), and **global_helper** (`cache()`, `auth()`, `config('key')`, etc., resolved via the global helper catalog). Dynamic calls are skipped. Summary lines: `Static service_location edges`, `Static facade edges`, and `Static global_helper edges`.
