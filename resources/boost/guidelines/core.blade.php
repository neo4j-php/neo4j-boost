...
Env vars for direct Neo4j connection: set `NEO4J_URI` (and user/password), or set only `NEO4J_DEFAULT_CONNECTION_DSN` (e.g. `neo4j://user:pass@neo4j-core1:7687` in Docker) so the same DSN as the app can be reused. Runtime resolution uses `(:Route)-[:HANDLED_BY]->(:Abstract)-[:RESOLVES_TO]->(:Instance)-[:DEPENDS_ON]->(:Dependency)-[:IDENTIFIED_AS]->(:Abstract)`, `(:Route)-[:USES_MIDDLEWARE]->(:Middleware)-[:IDENTIFIED_AS]->(:Abstract)`, `(:Event)-[:HANDLED_BY]->(:Abstract)`, `(:Job)-[:HANDLED_BY]->(:Abstract)`, `(:Job)-[:USES_CONNECTION]->(:QueueConnection)`, `(:ScheduledTask)-[:HANDLED_BY]->(:Abstract)`, `(:AuthGuard)-[:USES_PROVIDER]->(:AuthProvider)`, `(:AuthProvider)-[:USES_MODEL]->(:Abstract)`, `(:PasswordBroker)-[:USES_PROVIDER]->(:AuthProvider)`, `(:Policy)-[:HANDLED_BY]->(:Abstract)`, `(:Policy)-[:FOR_MODEL]->(:Abstract)`, `(:GateAbility)-[:HANDLED_BY]->(:Abstract)`, `(:Notification)-[:HANDLED_BY]->(:Abstract)`, `(:Notification)-[:USES_CHANNEL]->(:NotificationChannel)`, `(:Mailable)-[:HANDLED_BY]->(:Abstract)`, `(:Mailable)-[:USES_MAILER]->(:Mailer)`, `(:Mailable)-[:USES_CONNECTION]->(:QueueConnection)`, and `(:BroadcastChannel)-[:HANDLED_BY]->(:Abstract)`. All lookup names use `:Abstract` with a `kind` property (`Class`/`Interface`/`AbstractType`); bindings use `BINDS_TO`. Explore with:
...
```cypher
MATCH (e:Event)-[:HANDLED_BY]->(:Abstract)-[:RESOLVES_TO]->(root:Instance)
OPTIONAL MATCH path = (root)-[:DEPENDS_ON|IDENTIFIED_AS|RESOLVES_TO*0..8]->(n)
RETURN e, root, path LIMIT 50;
```
...
OPTIONAL MATCH path = (root)-[:DEPENDS_ON|IDENTIFIED_AS|RESOLVES_TO*0..8]->(n)
RETURN g, p, root, model, path LIMIT 50;
```
...
OPTIONAL MATCH path = (root)-[:DEPENDS_ON|IDENTIFIED_AS|RESOLVES_TO*0..8]->(n)
RETURN p, root, model, path LIMIT 50;
```
...
RETURN n, root, ch, path LIMIT 50;
```
...
**Dependency model:** `Route-[:HANDLED_BY]->Abstract-[:RESOLVES_TO {lifetime}]->Instance-[:DEPENDS_ON {file,line,via,type,method,parameter,helper,source,confidence,provenance,remarks,catalog_source}]->Dependency-[:IDENTIFIED_AS]->Abstract`, `Route-[:USES_MIDDLEWARE {order,parameters}]->Middleware-[:IDENTIFIED_AS]->Abstract`, `Event-[:HANDLED_BY]->Abstract`, `Job-[:HANDLED_BY]->Abstract`, `Job-[:USES_CONNECTION]->QueueConnection`, `ScheduledTask-[:HANDLED_BY {action}]->Abstract`, `AuthGuard-[:USES_PROVIDER]->AuthProvider`, `AuthProvider-[:USES_MODEL]->Abstract`, `PasswordBroker-[:USES_PROVIDER]->AuthProvider`, `Policy-[:HANDLED_BY]->Abstract`, `Policy-[:FOR_MODEL]->Abstract`, `GateAbility-[:HANDLED_BY]->Abstract`, `Notification-[:HANDLED_BY]->Abstract`, `Notification-[:USES_CHANNEL]->NotificationChannel`, `Mailable-[:HANDLED_BY]->Abstract`, `Mailable-[:USES_MAILER]->Mailer`, `Mailable-[:USES_CONNECTION]->QueueConnection`, and `BroadcastChannel-[:HANDLED_BY]->Abstract`. `DEPENDS_ON.type`: `constructor_injection`, `method_injection` (controller actions, `handle()` on jobs/commands/listeners/middleware, `via()` / `to*` on notifications, `build()` / `envelope()` / `content()` / `attachments()` on mailables, `join()` on broadcast channels), `global_helper`, or `instantiation` (direct `new ClassName()`). `DEPENDS_ON.catalog_source` (facade edges): `laravel_facade` for predefined Laravel facades, `auto_discovered_facade` for custom app facades and real-time facades. `Dependency.access`: `di`, `facade`, `global_helper`, `service_location`. `RESOLVES_TO.lifetime`: `singleton` or `bind`. MCP dependency entries expose `access`, `lifetime`, `source`, `confidence`, `provenance`, optional `remarks`, method-injection metadata when present, `helper` for global helpers, `type` for instantiation, and `catalog_source` for facade edges. Responses include `graph_completeness: partial` with known limitations. Bindings: `BINDS_TO` with `normal` or `singleton` plus metadata on `:Abstract` nodes. Contextual `when/needs/give` overrides: `Instance-[:CONTEXTUAL_BINDS {needs,needs_kind,reason}]->Abstract`. Re-run `container:graph` after upgrades.
