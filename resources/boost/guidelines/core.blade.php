...
**Dependency model (SOFT-58):** `Instance-[:DEPENDS_ON {file,line,via,type,method,parameter}]->Dependency-[:RESOLVES_TO {lifetime}]->Identifier`. `DEPENDS_ON.type`: `constructor_injection` or `method_injection` (controller actions, `handle()` on jobs/commands/listeners/middleware). `Dependency.access`: `di`, `facade`, `global_helper`, `service_location`. `RESOLVES_TO.lifetime`: `singleton` or `bind`. MCP dependency entries expose `access`, `lifetime`, and method-injection metadata when present. Bindings: `BINDS_TO` with `normal` or `singleton` on `:Abstract` nodes — re-run `container:graph` after upgrades.
