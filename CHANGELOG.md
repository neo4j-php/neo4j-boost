...
- **SOFT-47:** Static scan for Laravel global helper usage (`cache`, `auth`, `view`, `response`, `redirect`, `route`, `event`, `dispatch`, `logger`, `session`; `config`/`env` literal-key resolution). Exports `DEPENDS_ON` edges with `type: global_helper` and `helper`.
